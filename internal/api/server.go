...
func NewServer(cfg *config.Config, version string, out io.Writer, authHandler *auth.AuthHandler, storageHandler *storage.StorageHandler, realtimeHandler *realtime.RealtimeHandler, functionHandler *functions.FunctionHandler, policyHandler *permissions.PolicyHandler, openapiHandler *openapi.OpenAPIHandler, deployHandler *deploy.DeployHandler, backupHandler *backup.BackupHandler) *Server {
	...
	router := NewRouter(handler, authHandler, storageHandler, realtimeHandler, functionHandler, policyHandler, openapiHandler, deployHandler, backupHandler, out, DefaultTimeout)
...
===================================================================
  ...
  Endpoints   :
    ...
    - POST   /api/backup/create                     (Create .npb Server Backup - Admin)
    - POST   /api/backup/restore                    (Restore Full Server - Admin)
    - GET    /api/backup/list                       (List Backup Snapshots - Admin)
    - DELETE /api/backup/{id}                       (Delete Backup Snapshot - Admin)
    - GET    /api/backup/status                     (Backup Engine Status - Admin)
===================================================================
